...
# constsigma parameter order: Sigma0, A_IA, m0,m1,m2,m3,m4, b0,b1
PARAM_NAMES = ["Sigma0", "A_IA", "m_src0", "m_src1", "m_src2", "m_src3", "m_src4", "b_lens0", "b_lens1"]
N_PARAMS = len(PARAM_NAMES)
# Parameter bounds, fully aligned to Phase4B
BOUNDS = [
    (-0.5, 0.5),   # Sigma0: corresponds to s ∈ [1, 3], s/2 ∈ [0.5, 1.5] → Σ₀ = s/2 - 1 ∈ [-0.5, 0.5]
...
def predict(theta):
    ...
    # Pee scaling: (1+mean_m) * (1+Sigma0)^2
    pee = pee_baseline_lcdm * (1 + mean_m) * ((1 + Sigma0) ** 2)
    # Pne scaling: multiply by b_i * (1 + Sigma0) per lens bin
    pne = pne_baseline.copy()
    pne[mask_lens0] *= b0 * (1 + Sigma0)
    pne[mask_lens1] *= b1 * (1 + Sigma0)
...
# Print results
with open(outdir / "constsigma_fit_corrected.json", "w") as f:
