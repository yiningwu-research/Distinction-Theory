...
def main():
    ...
    if is_extended:
        ...
        nlive = args.nlive or 800
        dlogz = args.dlogz if args.dlogz is not None else 0.1
    ...
    try:
        ...
        raise ImportError("Install dynesty: pip install dynesty")
    ...
    tag = f"{args.model}_seed{args.seed}" if args.seed else args.model
    prior_label = (
        Path(args.config).stem.replace("nested_priors_", "")
        if args.config else "default"
    )
    tag = f"{tag}_{prior_label}"
    np.save(outdir / "chains" / f"{tag}_dynesty_samples.npy", res.samples)
    np.save(outdir / "chains" / f"{tag}_dynesty_logwt.npy", res.logwt)
    np.save(outdir / "chains" / f"{tag}_dynesty_logl.npy", res.logl)
    ...
    with open(outdir / "tables" / f"{tag}_nested_evidence.json", "w") as f:
